...
function Dashboard() {
  ...
  function getRiskConfig(risk) {
    const configs = {
      low: { label: t('risk.lowRisk'), badge: 'bg-emerald-50 text-emerald-700 border border-emerald-100 dark:bg-emerald-900/30 dark:text-emerald-300 dark:border-emerald-800', Icon: ShieldCheck, dot: 'bg-emerald-400' },
      medium: { label: t('risk.mediumRisk'), badge: 'bg-amber-50 text-amber-700 border border-amber-100 dark:bg-amber-900/30 dark:text-amber-300 dark:border-amber-800', Icon: AlertTriangle, dot: 'bg-amber-400' },
      high: { label: t('risk.highRisk'), badge: 'bg-red-50 text-red-700 border border-red-100 dark:bg-red-900/30 dark:text-red-300 dark:border-red-800', Icon: AlertTriangle, dot: 'bg-red-400' },
      uncertain: { label: t('risk.uncertainRisk'), badge: 'bg-gray-100 text-gray-600 border border-gray-200 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-700', Icon: ShieldCheck, dot: 'bg-gray-400' },
    }
    ...
    <div className="min-h-screen">
          ...
          <div className="grid lg:grid-cols-12 gap-8 items-center">
            ...
            <div className="lg:col-span-7 space-y-6">
              ...
              <div className="animate-hero-entrance opacity-0">
                <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full backdrop-blur-sm border shadow-sm ${darkMode ? 'bg-white/10 border-white/10' : 'bg-white/80 border-primary-100'}`}>
              ...
              <div className="animate-hero-entrance-delay-1 opacity-0">
                <h1 className={`text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight leading-tight ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                  {t('dashboard.welcome')},{' '}
                  <span className="text-gradient-medical">{displayName}</span>
                </h1>
                <p className={`mt-3 text-lg max-w-xl leading-relaxed ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                  {hasScans
            ...
            <div className="lg:col-span-5 flex justify-center lg:justify-end animate-hero-entrance-delay-3 opacity-0">
              <div className="relative w-64 h-64 md:w-80 md:h-80">
                {/* Outer scan ring */}
                <div className={`absolute inset-0 rounded-full border ${darkMode ? 'border-white/10' : 'border-primary-200/40'}`} />
                <div className={`absolute inset-3 rounded-full border ${darkMode ? 'border-white/5' : 'border-primary-100/30'}`} />
                ...
                <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 z-10">
                  <div className="glass-strong px-4 py-2 rounded-full shadow-sm">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-bold text-primary-700 dark:text-primary-300 tracking-[0.12em] uppercase">{t('scanvisual.status')}</span>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12 space-y-8">
        ...
        {loading ? (
          ...
          <section className="animate-fade-in">
            <div className={`card p-8 text-center border-2 ${darkMode ? 'border-red-800 bg-red-900/20' : 'border-rose-100 bg-rose-50/30'}`}>
              ...
              <p className={`text-sm mb-3 ${darkMode ? 'text-red-300' : 'text-rose-700'}`}>{error}</p>
                      ...
                      <div className="mt-auto space-y-3">
                        <div>
                          <div className="flex items-center justify-between text-xs mb-1.5">
                            <span className={darkMode ? 'text-gray-500' : 'text-gray-400'}>{t('scancard.aiConfidence')}</span>
                            <span className="font-semibold text-primary-600 dark:text-primary-400">{(confidence * 100).toFixed(1)}%</span>
                          </div>
                        ...
                        <Link
                          to="/results"
                          state={{ result: scan }}
                          className="inline-flex w-full items-center justify-center gap-2 px-4 py-2.5 rounded-xl border text-sm font-medium transition-all duration-200 group-hover:shadow-sm border-primary-100 text-primary-700 hover:bg-primary-50 hover:border-primary-200 dark:border-primary-800 dark:text-primary-300 dark:hover:bg-primary-900/30"
                        >
        ...
        {/* ═══════════════════════════════════════════════════════════
            ...
            ═══════════════════════════════════════════════════════════ */}
        <section className="animate-fade-in-up">
          <h2 className={`text-xl font-bold mb-5 ${darkMode ? 'text-white' : 'text-gray-900'}`}>{t('dashboard.quickActions')}</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            >
              <div className="w-12 h-12 rounded-xl bg-emerald-50 dark:bg-emerald-900/30 flex items-center justify-center mb-4 group-hover:bg-emerald-100 dark:group-hover:bg-emerald-900/50 transition-colors group-hover:scale-110 transition-transform duration-300">
                <History className="w-6 h-6 text-emerald-600 dark:text-emerald-400" />
              </div>
              <h3 className={`text-base font-semibold mb-1 ${darkMode ? 'text-white' : 'text-gray-900'}`}>{t('dashboard.viewHistory')}</h3>
              <p className={`text-xs leading-relaxed ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>{t('dashboard.viewHistoryDesc')}</p>
            ...
            <button
              ...
              className="group card-interactive p-6 text-left"
            >
              <div className="w-12 h-12 rounded-xl bg-accent-50 dark:bg-accent-900/30 flex items-center justify-center mb-4 group-hover:bg-accent-100 dark:group-hover:bg-accent-900/50 transition-colors group-hover:scale-110 transition-transform duration-300">
                <BarChart3 className="w-6 h-6 text-accent-600 dark:text-accent-400" />
              </div>
              <h3 className={`text-base font-semibold mb-1 ${darkMode ? 'text-white' : 'text-gray-900'}`}>{t('dashboard.compareScans')}</h3>
              <p className={`text-xs leading-relaxed ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>{t('dashboard.compareScansDesc')}</p>
              <ArrowUpRight className="w-4 h-4 mt-3 text-gray-300 group-hover:text-primary-500 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all" />
            </button>

            {/* Find Dermatologists — opens modal directly */}
            <button
              onClick={() => setDermModalOpen(true)}
              className="group card-interactive p-6 text-left"
            >
              <div className="w-12 h-12 rounded-xl bg-medical-50 dark:bg-medical-900/30 flex items-center justify-center mb-4 group-hover:bg-medical-100 dark:group-hover:bg-medical-900/50 transition-colors group-hover:scale-110 transition-transform duration-300">
                <MapPin className="w-6 h-6 text-medical-600 dark:text-medical-400" />
              </div>
              <h3 className={`text-base font-semibold mb-1 ${darkMode ? 'text-white' : 'text-gray-900'}`}>{t('dashboard.findDermatologists')}</h3>
              <p className={`text-xs leading-relaxed ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>{t('dashboard.findDermatologistsDesc')}</p>
              <ArrowUpRight className="w-4 h-4 mt-3 text-gray-300 group-hover:text-primary-500 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all" />
            </button>
          </div>
        </section>
