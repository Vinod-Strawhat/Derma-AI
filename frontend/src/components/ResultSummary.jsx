    ...
    <div className="card p-6 md:p-8">
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs font-medium uppercase tracking-wide text-gray-400">{t('resultsummary.prediction')}</p>
            {isUncertain ? (
              <h2 className="text-xl md:text-2xl font-semibold text-gray-900">{t('resultsummary.resultUncertain')}</h2>
            ) : (
              <h2 className="text-xl md:text-2xl font-semibold text-gray-900">{prediction.className}</h2>
            )}
          </div>
        </div>
      ...
      {isUncertain ? (
        <div className="rounded-xl bg-gray-50 border border-gray-100 p-4 mb-5">
          <p className="text-sm text-gray-600 leading-relaxed">{t('resultsummary.uncertainMessage')}</p>
        </div>
      ) : (
        <div className="mb-5">
          <div className="flex items-end justify-between mb-2">
            <span className="text-sm text-gray-500">{t('resultsummary.modelConfidence')}</span>
      ...
      {isHighRisk && (
        <div className="rounded-xl bg-red-50 border border-red-200 p-4 mb-5">
          <p className="text-sm font-medium text-red-700 mb-1">{t('resultsummary.highRiskSignal')}</p>
